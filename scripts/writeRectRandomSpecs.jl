...
function generatePulseRectSpec(dirPath::AbstractString,
    ...
    asData = (ttOut, llOut,
              (;gn00 = convert.(MatType, shgIntMatWvOut),
               gn10 = convert.(MatType, shgIntMatWv_gn10)))
...
targetDir = "/home/kjuen/dvlp/kai/data/simulatedTraces_7"
...
@time begin
    while success < nFiles
        ...
        local p = joinpath(targetDir, "s"*string(nameOffset + success+1))
        simple= rand() < 1/300
